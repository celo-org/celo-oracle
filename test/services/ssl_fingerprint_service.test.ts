...
import { SSLFingerprintService, SSLFingerprintServiceConfig, formatFingerprint } from "../../src/services/SSLFingerprintService";
...
describe('SSLFingerprintService', () => {
  ...
  describe('#updateFingerprint', () => {
    ...
    it('when the event object is invalid, it logs', () => {
      ...
      expect(service['logger'].error).toHaveBeenCalledWith(`Failed to process FingerprintUpdated event`)
      expect(service['logger'].error).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('when all is well, it logs and updates the mapping', () => {
      ...
      expect(service['logger'].info).toHaveBeenCalledWith(`Updating ${Exchange.OKX} fingerprint to ${formattedFingerprint}`)
