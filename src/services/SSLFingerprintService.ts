...
export class SSLFingerprintService implements ISSLFingerprintService {
  ...
  async init() {
    const fingerprints = await this.registry.methods.getFingerprints(ALL_EXCHANGE_IDENTIFIERS).call();
  ...
  updateFingerprint = (error: any, event: any) => {
    ...
    try {
      if (!event.returnValues || !event.returnValues.fingerprint || !event.returnValues.identifier) {
